...
import { Buffer } from "buffer"; // Add this line to import the Buffer polyfill
...
class Fetch {
  ...
  static async getRgbTile(uri, isNode, res) {
    const gp = getPixelsDom;
    console.log(getPixelsDom);
    // const gp = isNode ?
  ...
  static getZoomposEle(zpArray) {
    ...
    return Object.keys(elevations).map((triplet) =>
      triplet.split(",").map((num) => parseFloat(num))
    );
  }
